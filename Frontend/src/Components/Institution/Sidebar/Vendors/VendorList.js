...
const Vendors = () => {
  ...
  const vendors = useMemo(() => [
    { name: 'Ram Pharma', address: 'Indore', contact: '-', rating: 3.9, remarks: '-' },
    { name: 'Indore Pharma', address: 'Indore', contact: '-', rating: 4.0, remarks: '-' },
    { name: 'Krishna Pharma', address: 'Indore', contact: '-', rating: 3.6, remarks: '-' },
    { name: 'Indian Pharma', address: 'Indore', contact: '-', rating: 4.1, remarks: '-' },
  ], []);
    ...
    <div className="p-8">
      ...
      <div className="flex items-center mb-4">
        ...
        <button
          ...
          className={`px-4 py-2 mr-2 border ${sortKey === 'name' ? 'bg-gray-300' : ''}`}
        >
          Alphabet <FaSort className="inline ml-2" />
        </button>
        <button
          onClick={() => setSortKey('rating')}
          className={`px-4 py-2 border ${sortKey === 'rating' ? 'bg-gray-300' : ''}`}
        >
